...
<ol id="${input}Preview"></ol><a href="javascript:" onclick="${input}FinderOpen();" class="btn${disabled eq 'true' ? ' disabled' : ''}">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn${disabled eq 'true' ? ' disabled' : ''}">清除</a>
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		var url = "${ctxStatic}/js/ckfinder/ckfinder.html?type=${ctype}&start=${ctype}:${uploadPath}/"+year+"/"+month+
